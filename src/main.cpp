...
static SavedSettings LoadSettings() {
    ...
    std::wstring path = GetSettingsPath();
    if (path.empty()) return ss;

    std::ifstream f(path);
    if (!f.is_open()) return ss;
    ...
    if (gotX && gotY && gotW && gotH && ss.w > 0 && ss.h > 0) {
        ...
        if (GetMonitorInfo(hMon, &mi)) {
            RECT& work = mi.rcWork;
            if (ss.x + ss.w > work.right)  ss.x = work.right  - ss.w;
            if (ss.y + ss.h > work.bottom) ss.y = work.bottom - ss.h;
            if (ss.x < work.left) ss.x = work.left;
            if (ss.y < work.top)  ss.y = work.top;
            if (ss.w > work.right - work.left)  ss.w = work.right  - work.left;
            if (ss.h > work.bottom - work.top)  ss.h = work.bottom - work.top;
        }
        ss.windowValid = true;
    }
...
static void UpdateScrollInfo(HWND hwnd) {
    ...
    if (g_app->contentHeight <= clientHeight) {
        ...
        if (g_app->scrollOffset > maxScroll) g_app->scrollOffset = maxScroll;
...
static void TogglePin(HWND hwnd) {
    ...
    SetWindowPos(hwnd,
        g_app->isPinned ? HWND_TOPMOST : HWND_NOTOPMOST,
        0, 0, 0, 0,
    ...
    if (hSysMenu) {
        CheckMenuItem(hSysMenu, IDM_PIN_TO_TOP,
                      MF_BYCOMMAND | (g_app->isPinned ? MF_CHECKED : MF_UNCHECKED));
    }
    ...
    try {
        switch (msg) {
            ...
            case WM_LBUTTONDOWN: {
                ...
                ScrollHitZone zone = g_app->renderer->HitTestScrollbar(
                    mx, my, cw, ch, g_app->contentHeight, g_app->scrollOffset);
                ...
                if (zone == ScrollHitZone::Thumb) {
                    ...
                    int currentThumbY = (maxScroll > 0)
                        ? (int)((long long)g_app->scrollOffset * trackRange / maxScroll) : 0;
            ...
            case WM_MOUSEMOVE: {
                ...
                if (g_app->scrollDragging) {
                    ...
                    int newOffset = g_app->renderer->ScrollOffsetFromThumbDrag(
                        my, g_app->dragStartMouseY, g_app->dragStartOffset,
                        ch, g_app->contentHeight);
            ...
            case WM_USER + 1: {
                ...
                if (g_app && !g_app->initialResizeDone && g_app->contentHeight > 0) {
                    ...
                    AdjustWindowRectEx(&desired, GetWindowLong(hwnd, GWL_STYLE),
                    ...
                    SetWindowPos(hwnd, nullptr, 0, 0, curWidth, newHeight,
...
static INT_PTR CALLBACK UrlDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM) {
    switch (msg) {
        case WM_INITDIALOG: {
            ...
            SetWindowPos(hDlg, nullptr, (sx - w) / 2, (sy - h) / 2, 0, 0,
...
static INT_PTR ShowUrlInputDialog(HINSTANCE hInst, const std::wstring& defaultUrl,
    ...
    AddWord(0); AddWord(0);        // x, y (ignored with DS_CENTER)
    AddWord(280); AddWord(105);    // cx, cy (dialog units)
    AddWord(0);                     // menu
    ...
    AddDword(0);                    // exStyle
    AddWord(10); AddWord(7);       // x, y
    AddWord(260); AddWord(28);     // cx, cy
    AddWord(102);                   // ID
    AddWord(0xFFFF); AddWord(0x0082); // class: Static
    ...
    AddDword(0);
    AddWord(10); AddWord(38);
    AddWord(260); AddWord(10);
    AddWord(103);
    AddWord(0xFFFF); AddWord(0x0082);
    ...
    AddDword(0);
    AddWord(10); AddWord(52);
    AddWord(260); AddWord(14);
    AddWord(101);
    AddWord(0xFFFF); AddWord(0x0081); // class: Edit
    AddWord(0);                        // title (empty - set in WM_INITDIALOG)
    AddWord(0);
    ...
    AddDword(0);
    AddWord(165); AddWord(78);
    AddWord(50); AddWord(16);
    AddWord(IDOK);
    AddWord(0xFFFF); AddWord(0x0080); // class: Button
    AddWstr(L"OK");
    AddWord(0);
    ...
    AddDword(0);
    AddWord(220); AddWord(78);
    AddWord(50); AddWord(16);
    AddWord(IDCANCEL);
    AddWord(0xFFFF); AddWord(0x0080);
    AddWstr(L"Cancel");
    AddWord(0);

    return DialogBoxIndirectParamW(hInst, (DLGTEMPLATE*)buf.data(),
                                   nullptr, UrlDlgProc, 0);
}
...
static bool PromptAndValidateUrl(HINSTANCE hInst, HttpClient& client,
                                 std::wstring& outUrl, const std::wstring& defaultUrl) {
    std::wstring prompt;
    std::wstring currentUrl = defaultUrl;

    while (true) {
        INT_PTR res = ShowUrlInputDialog(hInst, currentUrl, prompt);
        if (res != IDOK) return false; // user cancelled

        currentUrl = g_dialogUrl;

        // Trim whitespace
        while (!currentUrl.empty() && currentUrl.front() == L' ') currentUrl.erase(currentUrl.begin());
        while (!currentUrl.empty() && currentUrl.back() == L' ')  currentUrl.pop_back();
...
bool ParseCommandLine(AppState& app, bool& debugMode, std::wstring& outUrlArg) {
    ...
    // Parse theme
    if (themeArg == L"light")       app.themeMode = ThemeMode::Light;
    else if (themeArg == L"dark")   app.themeMode = ThemeMode::Dark;
    else                            app.themeMode = ThemeMode::System;
